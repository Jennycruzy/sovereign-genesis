...
export async function GET() {
  ...
  try {
    ...
    const filteredEvents = events.filter(Boolean).slice(-20).reverse();
      ...
      return {
        prId:
          typeof args[0] === "object"
            ? args[0].hash || String(args[0])
            : String(args[0]),
        amount: ethers.formatEther(args[1]) + " XTZ",
      };
    case "BountyReleased":
      return {
        prId: args[0],
        contributor: args[1],
        amount: ethers.formatEther(args[2]) + " XTZ",
      };
...
function demoData() {
  return {
    ...
    events: [
      {
        name: "Received",
        args: { from: "0xDev1…", amount: "5.0 XTZ" },
        blockNumber: 100,
        txHash: "0xabc…",
      ...
      {
        name: "BountyPosted",
        args: { prId: "owner/repo#1", amount: "2.0 XTZ" },
        blockNumber: 101,
        txHash: "0xdef…",
      ...
      {
        name: "SurplusInvested",
        args: { amount: "3.0 XTZ", target: "0xYield…" },
        blockNumber: 103,
        txHash: "0xjkl…",
